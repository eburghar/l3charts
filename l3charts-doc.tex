...
\hypersetup{
  ...
  pdfauthor={Éric BURGHARD<eric@itsufficient.me>},
  pdfsubject={TikZ charts using LaTeX environments and expl3},
  pdfkeywords={LaTeX, TikZ},
    ...
    pdflicenseurl={https://github.com/eburghar/l3charts},
    pdflang={en-GB}
  }
...
\def\githuburl{https://github.com/eburghar/l3charts}

\begin{document}
\newgeometry{total={160mm,257mm},left=25mm,top=30mm}
...
\newgeometry{total={150mm,257mm},left=45mm,top=20mm}

\section{About this documentation}

I doubt that \LaTeX{} will have one day a documentation system as powerful as \href{https://doc.rust-lang.org/cargo/commands/cargo-doc.html}{\texttt{cargo doc}} due to its
...
\href{https://github.com/schlcht/microtype}{\pkg{microtype}} package which by the way, pushed
...
is comma separated list keyword and values :
...
passed to TikZ to draw the nodes containing the labels.
...
Use \opt{label-cs} to call \cs{textinv} to format the labels.

\DescribeMacro{textinv}
Macro used to format labels

\begin{verbatim}
% put a white text on a black background
\NewDocumentCommand\textinv{m}{%
  \colorbox{black}{\textcolor{white}{#1}}}
\end{verbatim}

\begin{verbatim}
% the scale option is passed to tikzpicture
\begin{kiviatchart}[scale=0.75]
  % we define all the dimentions of the charts, and specify the placement
  % of labels relatively to the nodes
  \begin{dims}[label-cs=textinv]
    ...
    \value[right]{Dim 5}
  \end{dims}
  % Then we can add one or several sets. Each value correspond to
  % the dimension at the same index in dims environment
  \begin{set}
    \value{4}
    \value{3}
    \value{4}
...
\begin{kiviatchart}[scale=0.75]
  \begin{dims}
    ...
    \value[right]{Dim 5}
  \end{dims}
  % fill this set in red with rectangle dots
  \begin{set}[fill=red,dot-options={fill,rectangle,inner sep=2pt}]
    ...
    \value{2}
  \end{set}
  % fill this set in green with diamond dots
  \begin{set}[fill=green,dot-options={fill,diamond,inner sep=2pt}]
    ...
    \value{5}
  \end{set}
  % fill this set in blue with semicircle dots
...
\begin{itemize}
  ...
  \item \cs{value} in \env{set} is between 0 and \opt{units}
\end{itemize}
...
Vertical separator in cm

\Describe{Option}{h-sep}{:fp}[0.5]
Horizontal separator (circle) in cm

\Describe{Option}{radius}{:fp}[0.25]{radius}
Radius of the circles in cm

\Describe{Option}{gap}{:fp}[0.05]
Gap between circle in cm

\Describe{Option}{label-cs}{:str}[identity]
cs name to format labels
...
\begin{verbatim}
% draw 5 circles bar hidding the circles
...
\begin{verbatim}
% format the labels, fill in red and shows circles
\begin{ballchart}[n=6, label-cs=textinv, v-sep=0.2, fill-options={fill=red!50}, draw-options={draw=black}]
  \value{Dim 1}{95}
  \value{Dim 2}{80}
  \value{Dim 3}{80}
  \value{Dim 4}{20}
...
maximum width (\textbf{required}) in cm
...
\Describe{Option}{fill-options}{:prop}[\{fill=black\}]
TikZ option to fill the bar with

\Describe{Option}{draw-options}{:prop}[\{draw=black!20\}]
TikZ option to draw the bar with
...
\begin{verbatim}
% hide borders
\begin{barchart}[draw-options={draw=none}]
  \value{Dim 1}{60}
  \value{Dim 2}{100}
  \value{Dim 3}{70}
  \value{Dim 4}{70}
...
\begin{verbatim}
% draw 3cm wide bars, format labels, fill in red and show borders 
...
\begin{verbatim}
% formate labels, fill in red, and show maximum dashed
...
\item define a document class borrowed to \href{https://github.com/schlcht/microtype}{\pkg{microtype}}
