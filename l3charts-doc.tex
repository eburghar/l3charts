...
\Describe{Option}{radius}{:dim}[=kiviatchart radius]
...
Position of the label. Should be \texttt{left}, \texttt{right}, \texttt{above}, \texttt{below}, \texttt{above right}, \texttt{above left}, \texttt{below right} or  \texttt{below left}
...
\section{Circle chart}

\subsection{Usage}

\DescribeEnv{circlechart}
Environment that hold a circle chart. Accepts an optional argument \oarg{clist} which
is comma separated list of keywords and values :
...
Line width to draw the circles with
...
\Describe{Option}{label-pos}{:str}[right]
Label position relative to radial. Should be \texttt{left} \texttt{center} or \texttt{right}
...
\item add a \env{circlechart}
\item use choice to restrict values on several keys (\opt{label-pos})
